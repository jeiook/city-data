...
class ZoriMetroDataExplorer:
    def __init__(self, filename: str) -> None:
        self._dataExtractor = DataExtractor(
            CsvFileSourceDataRetriever(filename),
            DictionaryDataObjectBuilder()
        )
        self._dataDict = {data.getValueAtProperty(
            "RegionName"): data for data in self._dataExtractor}

    def _getRegionNameFromCityState(self, city: str, stateAbbrev: str) -> str:
        if " " in city:
    ...
    def getRentsOfRegionOverRange(self, city: str, stateAbbrev: str, startDate: date = date.fromisoformat("2015-01-31"), endDate: date = date.fromisoformat("2024-02-29")):
        data = self._dataDict[self._getRegionNameFromCityState(
            city, stateAbbrev)]
        rents = []
        startFound = False
